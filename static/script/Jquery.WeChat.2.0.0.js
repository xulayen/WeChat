 ...
 * 此版本为最终版本，此项目将不会再维护！2020/6/9 15:42:58
 */
; (function (root, factory) {
    ...
    var WX = weixin || this.jWeixin || require('weixin-js-sdk');
    /**
     * 锁定当前目录，为了解决苹果手机单页应用路由变化之后，路由地址还是之前的地址，从而导致获取的签名不对无法使用JSSDK API的问题
     */
    ...
    var WXAPIConfig = {
        ...
        api: '',
        data: {},
        scanAuthUrl: isIOS() ? window.entryUrl : window.location.href,
        hideOptionMenu: true,
        async: false, //是否异步
        type: 'POST',
    ...
    /**
     * 
     * @param {*} options 微信JSSDK必要的参数  appid、timestamp、nonceStr、signature
     */
    async function WeChart(options) {
        var _self = this;
        opts = $.extend({}, WXAPIConfig, options);
        opts.data.url = WXAPIConfig.scanAuthUrl;
        await PatchWechatData();
        return _self;
    };

    async function PatchWechatData() {
        if (opts.api) {
        ...
        if (!opts.appId || !opts.timestamp || !opts.nonceStr || !opts.signature) {
            d.error(' please init appid、timestamp、nonceStr、signature parameters first. ');
            d.lookDebug(opts);
            return;
        }
     ...
     * @constructor
     */
    $.fn.ChooseImg = $.ChooseImg = function (option, success, fail, complete) {
        var _self = this;
        try {
            WX.ready(function () {
                WX.chooseImage({
                    ...
                    fail: function () {
                        fail && fail.call(_self);
                    },
                    complete() {
                        complete && complete.call(_self);
                    }
    ...
    /****
     * 获取图片
     * @returns {$.fn.GetLocalImgData}
     */
    $.fn.GetLocalImgData = $.GetLocalImgData = function (localId, success) {
        var _self = this;
        try {
            WX.ready(function () {
                WX.getLocalImgData({
     ...
     * @type {Function}
     */
    $.fn.GetLocation = $.GetLocation = function (type, success, cancel) {
        var _self = this;
        try {
            WX.ready(function () {
                WX.getLocation({
     ...
     * 微信支付
     * @type {Function}
     * @param {pay} timestamp、nonceStr、package、signType、paySign
     **/
