...
;(function (root, factory) {
    ...
    var WXAPIConfig = {
        ...
        baseapi_startSearchBeacons:true,
        baseapi_stopSearchBeacons:true,
        baseapi_onSearchBeacons:true,
        appId: null,
        timestamp: null,
        nonceStr: null,
        signature: null,
    ...
    function WeChart(options) {
        var _self = this;
        WXAPIConfig.data.url=WXAPIConfig.scanAuthUrl;
        WXAPIConfig.data.typenum=options.typenum;
        WXAPIConfig.data.facid=options.facid;
    ...
    $.fn.InitWeChat = $.InitWeChat = function (config, success, error) {
        ...
        try {
            ...
            if (opts.baseapi_startSearchBeacons) {
                jsApi.push('startSearchBeacons');
            }
            if (opts.baseapi_stopSearchBeacons) {
                jsApi.push('stopSearchBeacons');
            }
            if (opts.baseapi_onSearchBeacons) {
                jsApi.push('onSearchBeacons');
            }
     ...
     * 监听周边ibeacon设备接口
     */
    $.fn.onSearchBeacons = $.onSearchBeacons = function (callback) {
        var img=new Image();
        WX.startSearchBeacons({
            ticket:"",
            complete:function(argv){
                img.src="http://10.20.26.19/dm/r5e3/server/R5E3.ashx?action=2&type=11111&e="+JSON.stringify(argv);
                WX.onSearchBeacons({
                    complete:function(argv){
                        callback && callback.call(_self, argv);
  
                        img.src="http://10.20.26.19/dm/r5e3/server/R5E3.ashx?action=2&type=222222&e="+JSON.stringify(argv);
                    }
                 });
            }
         });
